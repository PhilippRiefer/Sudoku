...
// Version: 1.2
// Datum: 23.05.2020
// Beschreibung: Ger�st f�r ein Sudoku-Programm mit Windows-Benutzeroberfl�che
//=============================================================================
...
#define BOX_SIZE 49		// Gr��e eines Feldes; der Wert sollte sein: 3n+1
#define GAP_SIZE 7		// extra Abstand f�r 9er-Block
				   ...
				   PSTR szCmdLine, int iCmdShow)
{
	...
	// Ein Fenster f�r die Applikation erstellen
	hWnd = CreateWindowEx(
		...
		3*Y_OFFSET+9*BOX_SIZE+2*GAP_SIZE+38+30,	// Fensterh�he
		NULL,						// �bergeordnetes Fenster
		NULL,						// Men�
		hInstance,					// Handle der Applikation
		NULL);						// keine zus�tzlichen Parameter
	...
	// Position der n�chsten Taste berechnen
...
{
	static int x1, y1, n1;	// Position im Sudoku beim Dr�cken der Maus
	int x2, y2, n2;			// Position im Sudoku beim Loslassen der Maus
	RECT rect;				// Region, die neu gezeichnet werden soll
	char Filter[] =			// Dateifilter f�r Dialog
		"Sudoku-Datei (*.sdk)\0*.sdk\0" "Alle Dateien (*.*)\0*.*\0";
	char FileName[301]="";	// Dateiname
	char title[20];			// Titel f�r Dialog
	OPENFILENAME FileData = {	// Informationen F�r Dateidialog:
	    sizeof(OPENFILENAME),// Gr��e dieser Struktur
	    hWnd,				// Eigent�mer-Fenster dieses Dialogs
	    0,					// Zugeh�rige Applikation
	    Filter,				// Dateifilter
	    NULL,				// Letzter gew�hlter Dateityp
	    0,					// L�ange des letzten gew�hllen Dateityps
	    0,					// Index des gew�hlten Dateityps
	    FileName,			// Dateiname mit Pfad
	    300,				// maximale L�nge des Dateinamens mit Pfad
	    NULL,				// Dateiname ohne Pfad
	    0,					// maximale L�nge des Dateinamens ohne Pfad
	...
	case WM_COMMAND:	// allgemeinder Befehl
		if(HIWORD(wParam)==BN_CLICKED) {
			...
			case LOAD_BUTTON_ID:	// die Taste 'Laden..' bet�tigt
			...
			case SAVE_BUTTON_ID:	// die Taste 'Speichern...' bet�tigt
			...
			case SPECIAL_BUTTON1_ID:// Spezialtaste 1 bet�tigt
				Sudoku(special1, 0, 0, 0, NULL, hWnd);
				RedrawWindow(hWnd, NULL, NULL, RDW_ERASE|RDW_INVALIDATE);
				break;

			case SPECIAL_BUTTON2_ID:// Spezialtaste 2 bet�tigt
				Sudoku(special2, 0, 0, 0, NULL, hWnd);
				RedrawWindow(hWnd, NULL, NULL, RDW_ERASE|RDW_INVALIDATE);
				break;

			case SPECIAL_BUTTON3_ID:// Spezialtaste 3 bet�tigt
...
void drawWindow(HWND hWnd)
{
	HDC hdc;		// Handle f�r Zeichenumgebung
...
void drawNumbers(HDC hdc, int xOff, int yOff, int boxSize, int gapSize)
{
	...
	HFONT bigThickFont;	// gro�e fette Schrift
	HFONT bigThinFont;	// gro�e d�nne Schrift
